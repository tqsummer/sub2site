  ...
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      ...
      <template v-if="qrUrl">
        ...
        <p v-if="scanHint" class="text-center text-sm text-content-2">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-content-2">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-if="expired" class="text-center">
        <p class="text-lg font-medium text-danger">{{ t('payment.qr.expired') }}</p>
      </div>
      <div v-else class="text-center">
        <p class="text-sm text-content-2">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
    ...
    <div v-else class="flex flex-col items-center space-y-4 py-4">
      ...
      <p class="text-lg font-bold text-content">{{ t('payment.result.success') }}</p>
      <div v-if="paidOrder" class="w-full rounded-xl bg-surface-2 p-4">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-content">{{ paymentAmountSymbol(paidOrder) }}{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
