    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-content-2">{{ t('admin.settings.payment.paymentMode') }}</span>
            ...
            <button
              ...
              :class="[
                ...
                form.payment_mode === mode.value
                  ...
                  : 'border-gray-300 bg-white text-content-2 hover:border-gray-400 hover:bg-gray-50 dark:border-dark-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:border-dark-500',
              ]"
            >{{ mode.label }}</button>
          </div>
        </div>
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-content-2">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div v-if="form.provider_key === 'easypay'" class="space-y-3 rounded-lg border border-divider p-3">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h5 class="text-sm font-medium text-content">
        ...
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            <div>
              <label class="text-xs text-content-2">{{ t('admin.settings.payment.customMethodType') }}</label>
              <input v-model="method.type" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-content-2">{{ t('admin.settings.payment.customMethodUpstreamType') }}</label>
              <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-content-2">{{ t('admin.settings.payment.customMethodDisplayName') }}</label>
            ...
            <button
              type="button"
              class="rounded-lg border border-red-200 px-2.5 py-2 text-xs font-medium text-danger transition-colors hover:bg-red-50 dark:border-red-800/60 dark:hover:bg-red-900/20"
      ...
      <div class="border-t border-divider pt-4">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-divider-strong text-[11px] font-semibold text-content-3 transition-colors hover:border-primary-500 hover:text-primary-600 dark:hover:border-primary-400 dark:hover:text-primary-400"
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-content-2">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-content-3">({{ t('common.optional') }})</span>
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-content-3 hover:text-content-2 dark:hover:text-gray-300"
              >
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-content-2">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-divider-strong bg-surface-2 px-3 text-xs text-content-2">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-divider-strong bg-surface-2 px-3 text-xs text-content-2">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-if="limitableTypes.length" class="border-t border-divider pt-4">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-content-3 transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          ...
          >
            <p class="mb-2 text-xs font-medium text-content-2">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-content-2">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-content-2">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-content-2">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-content-3">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
