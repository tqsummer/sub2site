<template>
  <div
    :class="[
      ...
      enabled ? 'border-divider' : 'border-gray-200 bg-gray-50 opacity-50 dark:border-dark-700 dark:bg-dark-800/50',
    ]"
  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-3">
        <div :class="[
          'rounded-md p-1.5',
          provider.enabled && enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-surface-3',
        ]">
          <Icon
            name="server"
            size="sm"
            :class="provider.enabled && enabled ? 'text-success' : 'text-content-3'"
          />
        </div>
        <span class="text-sm font-medium text-content">{{ provider.name }}</span>
        <span class="text-xs text-content-3">{{ keyLabel }}</span>
        <span v-if="provider.payment_mode" class="text-xs text-content-3">· {{ modeLabel }}</span>
          ...
          <button
            ...
            :class="[
              ...
              isSelected(pt.value)
                ...
                : 'bg-gray-100 text-content-3 dark:bg-dark-700',
            ]"
          >{{ pt.label }}</button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 border-l border-divider pl-3">
          <button type="button" @click="emit('edit')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400">
            <Icon name="edit" size="sm" />
            <span class="text-xs">{{ t('common.edit') }}</span>
          </button>
          <button type="button" @click="emit('delete')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
