<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <div v-if="loading" class="py-8 text-center text-sm text-content-2">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="!detail" class="py-8 text-center text-sm text-content-2">
      {{ t('channelStatus.detailLoadError') }}
    </div>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-divider">
          <tr class="text-xs uppercase tracking-wider text-content-2">
        ...
        <tbody>
          ...
          >
            <td class="py-2 pr-3 font-medium text-content">{{ formatMonitorModel(m.model) }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-content-2">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-content-2">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-content-2">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-content-2">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-content-2">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
