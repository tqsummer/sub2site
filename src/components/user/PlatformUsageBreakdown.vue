<template>
  <div class="group/usage relative text-sm">
    <div class="flex items-center gap-1.5">
      <span class="text-content-2">{{ t('admin.users.today') }}:</span>
      <span class="font-medium text-content">${{ today.toFixed(4) }}</span>
      <Icon
        v-if="hasBreakdown"
        name="infoCircle"
        size="xs"
    ...
    <div class="mt-0.5 flex items-center gap-1.5">
      <span class="text-content-2">{{ t('admin.users.total') }}:</span>
      <span class="font-medium text-content">${{ total.toFixed(4) }}</span>
    </div>
