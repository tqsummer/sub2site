  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-content-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-divider bg-surface p-3"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-content-3 hover:text-content-2 dark:hover:text-gray-300" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-content">{{ attr.name }}</span>
              <span class="rounded bg-surface-3 px-1.5 py-0.5 font-mono text-xs text-content-2">
                {{ attr.key }}
              </span>
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-content-2">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-content-2 hover:bg-gray-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-content-2 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-content-2 hover:bg-red-50 hover:text-red-600"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-divider-strong text-primary-600" />
          <span class="text-sm text-content-2">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-divider-strong text-primary-600" />
          <span class="text-sm text-content-2">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
