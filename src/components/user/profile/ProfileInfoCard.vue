    ...
    >
      <div class="px-6 py-6 md:px-8">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
          <div
            class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-token-xl bg-gradient-to-br from-primary-500 to-primary-600 text-2xl font-bold text-white shadow-lg shadow-primary-500/20"
          >
            <img
              v-if="avatarUrl"
              :src="avatarUrl"
          ...
          <div class="min-w-0 flex-1 space-y-5">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                <h2 class="truncate text-2xl font-semibold text-content">
                  {{ displayName }}
                </h2>
              ...
              <div class="space-y-1">
                <p class="truncate text-sm text-content-2">
                  {{ primaryEmailDisplay }}
                </p>
                <div
                  v-if="sourceHints.length"
                  class="flex flex-wrap gap-2 text-xs text-content-2"
                >
                  <span
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              <div
                ...
                class="rounded-token-lg bg-white/85 px-4 py-3 shadow-sm ring-1 ring-white/70 dark:bg-dark-900/60 dark:ring-dark-700"
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-content-3">
    ...
    <div class="space-y-6">
      <div data-testid="profile-main-column" class="space-y-6">
        <section
          ...
          class="card border border-divider bg-white/90 p-6 dark:bg-dark-900/50"
        >
          ...
          <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            <div class="rounded-token-xl border border-divider bg-surface-2 p-5">
              <ProfileAvatarCard
                :user="user"
                embedded
              />
        ...
        >
          ...
          <div class="mt-5 grid gap-3">
            <div
              v-for="hint in sourceHints"
              :key="hint.key"
              class="flex items-start gap-3 rounded-token-lg border border-divider bg-surface-2 px-4 py-3 text-sm text-content-2 dark:text-gray-300"
            >
              <Icon name="link" size="sm" class="mt-0.5 text-content-3" />
              <span>{{ hint.text }}</span>
            </div>
          </div>
        </section>
