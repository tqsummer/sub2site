    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          <div class="w-11 h-6 bg-surface-3 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 dark:peer-focus:ring-primary-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:after:border-gray-600 peer-checked:bg-primary-600"></div>
        </label>
      </div>
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-content-3 ml-2">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
            ...
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              ...
              <div class="flex items-center gap-2 min-w-0 flex-1">
                <label class="relative inline-flex items-center cursor-pointer shrink-0">
                  ...
                  <div class="w-9 h-5 bg-surface-3 peer-focus:outline-none rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:after:border-gray-500 peer-checked:bg-primary-600"></div>
                </label>
                <span class="text-sm text-content-2 truncate">{{ entry.email }}</span>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-divider-strong px-2 py-1 text-xs dark:bg-dark-700"
                    ...
                    <span v-if="verifyCountdown > 0" class="text-xs text-content-3">{{ verifyCountdown }}s</span>
                    <button v-else @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-content-2 hover:text-content-2">
                      {{ t('profile.balanceNotify.resend') }}
                    </button>
                    <button @click="verifyingEmail = ''" class="text-xs text-content-3 hover:text-content-2">
                      {{ t('common.cancel') }}
                    </button>
                  </template>
                  <template v-else>
                ...
                <button @click="handleRemoveEmail(entry.email)" class="text-danger hover:text-red-700 text-xs">
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div v-for="(pe, idx) in pendingEmails" :key="pe.email"
              class="flex items-center gap-2 px-3 py-2 bg-warning-soft rounded-lg border border-yellow-200 dark:border-yellow-800">
              <span class="flex-1 text-sm text-content-2">{{ pe.email }}</span>
              <div v-if="!pe.codeSent" class="flex items-center gap-1">
                ...
                <button @click="pendingEmails.splice(idx, 1)" class="text-xs text-danger hover:text-red-700 ml-1">
                  {{ t('profile.balanceNotify.removeEmail') }}
                </button>
              </div>
              <div v-else class="flex items-center gap-1">
                ...
                <span v-if="pe.countdown > 0" class="text-xs text-content-3">{{ pe.countdown }}s</span>
                <button v-else @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-content-2 hover:text-content-2">
                  {{ t('profile.balanceNotify.resend') }}
                </button>
              </div>
            </div>
          ...
          <p v-else class="text-xs text-content-3">
