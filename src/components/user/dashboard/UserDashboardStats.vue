<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="stat-value text-success">${{ formatBalance(balance) }}</p>
          <p class="text-content-3 text-xs">{{ t('common.available') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="stat-icon stat-icon-primary">
          <Icon name="key" size="md" class="" :stroke-width="2" />
        </div>
        <div>
          <p class="stat-label">{{ t('dashboard.apiKeys') }}</p>
          <p class="stat-value">{{ stats?.total_api_keys || 0 }}</p>
          <p class="text-success text-xs">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="stat-icon stat-icon-success">
          <Icon name="chart" size="md" class="" :stroke-width="2" />
        </div>
        <div>
          <p class="stat-label">{{ t('dashboard.todayRequests') }}</p>
          <p class="stat-value">{{ stats?.today_requests || 0 }}</p>
          <p class="text-content-3 text-xs">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
        </div>
      </div>
    </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="stat-value">
            ...
            <span class="text-content-3 text-sm font-normal" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-content-2">{{ t('common.total') }}: </span>
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">${{ formatCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-content-3" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="stat-icon stat-icon-warning">
          <Icon name="cube" size="md" class="" :stroke-width="2" />
        </div>
        <div>
          <p class="stat-label">{{ t('dashboard.todayTokens') }}</p>
          <p class="stat-value">{{ formatTokens(stats?.today_tokens || 0) }}</p>
          <p class="text-content-3 text-xs">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.today_cache_creation_tokens || 0) + (stats?.today_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-content-3 text-xs">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.total_cache_creation_tokens || 0) + (stats?.total_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
  ...
  <div v-if="!isSimple && platformCards.length > 0" class="card p-4">
    <div class="mb-3 flex items-center justify-between">
      <h3 class="text-content text-sm font-semibold">{{ t('dashboard.platformBreakdown') }}</h3>
      <span class="text-content-3 text-xs">
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          <span class="text-content text-sm font-semibold">
        ...
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-content-2">{{ t('dashboard.todayCost') }}</span>
            <span class="text-content font-mono">${{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-content-2">{{ t('dashboard.requests') }}</span>
            <span class="text-content-2 font-mono">
              {{ item.total_requests > 0 ? formatNumber(item.total_requests) : '-' }}
            </span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-content-2">{{ t('dashboard.tokens') }}</span>
            <span class="text-content-2 font-mono">
        ...
        <div v-if="hasAnyLimit(item.quota) && !item.isOther" class="mt-3 space-y-1.5 border-t pt-2">
          <p class="text-content-3 text-[10px] uppercase tracking-wide">
            ...
            <div v-if="quotaVal(item.quota, `${w}_limit_usd`) != null" class="space-y-0.5">
              ...
              <template v-if="(quotaVal(item.quota, `${w}_limit_usd`) as number) === 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-content-2">{{ t(`dashboard.platformQuota.${w}`) }}</span>
                  <span class="text-danger font-mono">{{ t('dashboard.platformQuota.disabled') }}</span>
                </div>
                <div class="h-1.5 w-full overflow-hidden rounded-full bg-surface-3">
                  <div class="bg-danger h-full w-full rounded-full" />
                </div>
              </template>
              <!-- limit>0：正常用量进度条 -->
              <template v-else>
                ...
                <p v-if="quotaVal(item.quota, `${w}_window_resets_at`)" class="text-content-3 text-[10px]">
