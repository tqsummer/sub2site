    ...
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <template #cell-model="{ row }">
          <span v-if="row.model" class="text-sm font-medium text-content">{{ row.model }}</span>
          <span v-else class="text-sm text-content-3">-</span>
        </template>

        <template #cell-key_name="{ row }">
          <div class="text-sm">
            <span class="text-content">{{ row.key_name || '-' }}</span>
            <span
              v-if="row.key_deleted"
              class="ml-1 inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-rose-100 text-danger ring-1 ring-inset ring-rose-200 dark:ring-rose-500/30"
        ...
        <template #cell-endpoint="{ row }">
          <div class="max-w-[320px] text-xs">
            <div class="break-all text-content-2">
              <span class="font-medium text-content-2">{{ t('usage.inbound') }}:</span>
        ...
        <template #cell-category="{ row }">
          <span class="text-sm text-content">{{ t('usage.errors.categories.' + row.category) }}</span>
        </template>

        <template #cell-message="{ row }">
          <span
            v-if="row.message"
            class="block max-w-[280px] truncate text-sm text-content-2"
            :title="row.message"
          >{{ row.message }}</span>
          <span v-else class="text-sm text-content-3">-</span>
        </template>
        ...
        <template #cell-platform="{ row }">
          <span class="text-sm text-content">{{ row.platform || '-' }}</span>
        </template>

        <template #cell-client_ip="{ row }">
          <div @click.stop>
            <div v-if="row.client_ip">
              <span class="text-sm font-mono text-content-2">{{ row.client_ip }}</span>
        ...
        <template #cell-created_at="{ row }">
          <span class="text-sm text-content-2">{{ formatDateTime(row.created_at) }}</span>
        </template>

        <template #cell-user_agent="{ row }">
          <span
            v-if="row.user_agent"
            class="block max-w-[320px] truncate text-sm text-content-2"
