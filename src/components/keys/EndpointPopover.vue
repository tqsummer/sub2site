...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-lg border border-divider bg-surface px-2.5 py-1.5 text-xs transition-colors hover:border-primary-200 dark:hover:border-primary-700"
    >
      <span class="font-medium text-content-2">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        ...
        <code
          class="cursor-pointer font-mono text-content-2 decoration-gray-400 decoration-dashed underline-offset-2 hover:text-primary-600 hover:underline focus:text-primary-600 focus:underline focus:outline-none dark:decoration-gray-500 dark:hover:text-primary-400 dark:focus:text-primary-400"
          role="button"
          tabindex="0"
        ...
        <button
          ...
          :class="copiedEndpoint === item.endpoint
            ? 'text-success'
            : 'text-content-3 hover:text-primary-500 dark:hover:text-primary-400'"
        ...
        <a
          ...
          class="rounded p-0.5 text-content-3 transition-colors hover:text-amber-500 dark:hover:text-amber-400"
