<template>
  ...
  >
    ...
    <header class="border-b border-divider px-5 py-4">
      ...
      <p v-if="group.description" class="mt-2 text-sm text-content-2">
        {{ group.description }}
      </p>
      <p
        v-if="peakNote"
        class="mt-1.5 inline-flex items-center gap-1 text-xs text-warning"
      >
      ...
      <p
        ...
        class="mt-1.5 flex items-center gap-1 text-xs text-content-2"
      >
    ...
    <div>
      <PlazaModelPricingTable
        ...
        :peak-rate-multiplier="group.peak_rate_multiplier"
      />
      <p v-else class="px-5 py-4 text-center text-sm text-content-3">
