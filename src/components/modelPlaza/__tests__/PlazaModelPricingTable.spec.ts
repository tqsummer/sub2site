...
/**
 * 列优先级降级：colgroup 声明的列数必须与数据行实际渲染的列数一致。
 * 这次改动同时动了 colgroup、双层表头 colspan、数据行三处，
 * 少改一处表格就会错位——这是最容易出错也最难肉眼发现的地方，单独锁死。
 */
describe('PlazaModelPricingTable 列优先级降级', () => {
  function countBodyCols(wrapper: ReturnType<typeof mountTable>): number {
    return wrapper
      .findAll('tbody tr')[0]
      .findAll('td')
      .reduce((n, td) => n + Number(td.attributes('colspan') || 1), 0)
  }

  it('默认（宽内容区）渲染全部 8 列，且表头与数据行列数一致', () => {
    const wrapper = mountTable([tokenModel()], 1)
    expect(wrapper.findAll('colgroup col')).toHaveLength(8)
    expect(countBodyCols(wrapper)).toBe(8)
  })
