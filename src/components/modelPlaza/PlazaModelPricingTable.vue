<template>
  <div ref="tableWrapRef" class="plaza-pricing-table overflow-x-auto" :style="accentStyle">
    <table :class="['w-full table-fixed border-collapse text-sm tabular-nums', showOfficial ? 'min-w-[1000px]' : showSiteCache ? 'min-w-[640px]' : 'min-w-[480px]']">
      <colgroup>
        <col :class="showOfficial ? 'w-[25%]' : showSiteCache ? 'w-[40%]' : 'w-[46%]'" />
        <col :class="showOfficial ? 'w-[11%]' : showSiteCache ? 'w-[16%]' : 'w-[20%]'" />
        <col :class="showOfficial ? 'w-[9%]' : showSiteCache ? 'w-[16%]' : 'w-[20%]'" />
        <col v-if="showSiteCache" :class="showOfficial ? 'w-[14%]' : 'w-[16%]'" />
        <template v-if="showOfficial">
          <col class="w-[11%]" />
          <col class="w-[8%]" />
          <col class="w-[14%]" />
        </template>
        <col :class="showOfficial ? 'w-[8%]' : 'w-[14%]'" />
      </colgroup>
      <thead>
        <tr
          class="text-xs font-semibold uppercase tracking-wider text-content-2"
        >
          <th
            rowspan="2"
            class="border-r border-divider py-2.5 pl-5 pr-4 text-left align-middle"
          >
            {{ t('modelPlaza.table.model') }}
          </th>
          <th :colspan="showSiteCache ? 3 : 2" class="pz-bg pt-2 text-center">
          ...
          <th
            v-if="showOfficial"
            colspan="3"
            class="border-l border-divider pt-2 text-center"
          >
            <div class="border-b border-divider pb-2 text-content-3">
              {{ t('modelPlaza.table.officialPrice') }}
              <span class="ml-1 normal-case font-normal text-content-3">{{ t('modelPlaza.table.unitPerMillion') }}</span>
            </div>
          </th>
          <th
            rowspan="2"
            class="border-l border-divider py-2.5 pl-3 pr-5 text-right align-middle"
          >
            {{ t('modelPlaza.table.rate') }}
          </th>
        </tr>
        <tr
          class="border-b border-divider text-left text-[11px] font-medium uppercase leading-4 tracking-wide text-content-3"
        >
          ...
          <th v-if="showSiteCache" class="pz-bg px-3 py-2 font-medium">
            {{ t('modelPlaza.table.cache') }}
          </th>
          <template v-if="showOfficial">
            <th class="border-l border-divider px-3 py-2 font-medium">
      ...
      <tbody>
        <tr
          ...
          :key="key"
          class="border-b border-divider transition-colors last:border-b-0 hover:bg-gray-50/70 dark:hover:bg-dark-800/50"
        >
          <!-- 模型名 + 非 token 计费模式徽章;分时时段行额外标注时段 -->
          <td class="border-r border-divider py-2.5 pl-5 pr-4 align-middle">
            <div class="flex flex-wrap items-center gap-1.5">
              <span class="font-medium text-content">{{ m.name }}</span>
              <!-- 时段徽章紧跟模型名,其余徽章排在后面,空间不足时先换行的是它们 -->
              <span
                v-if="period"
                class="inline-flex items-center whitespace-nowrap rounded-md bg-surface-3 px-1 py-0.5 font-mono text-[10px] font-medium text-content-2 dark:text-dark-300"
              ...
              <span
                ...
                class="rounded-md bg-surface-3 px-1.5 py-0.5 text-[10px] font-medium text-content-2 dark:text-dark-300"
              >
                {{ billingModeLabel(m) }}
              </span>
              <span
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-content dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                ...
                >
                  <span class="mr-1 font-sans font-normal text-content-3" :title="tierHint(m)">{{ tierLabel(iv) }}</span>
            ...
            <template v-if="showSiteCache">
            <td class="pz-cell px-3 py-2.5 align-middle">
              <template v-if="hasTierCachePricing(tokenIntervals(m))">
                <div
                  v-for="(iv, idx) in tokenIntervals(m)"
                  :key="idx"
                  class="whitespace-nowrap font-mono text-xs leading-5 text-content"
                  :title="tierHint(m)"
                >
                  <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                    <span class="font-sans font-normal text-content-3">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                    {{ paidPerMillion(iv.cache_write_price, period) }}
                    <template v-if="iv.cache_write_1h_price != null"
                      ><span class="font-sans font-normal text-content-3"> (1h </span>{{ paidPerMillion(iv.cache_write_1h_price, period)
                      }}<span class="font-sans font-normal text-content-3">)</span></template
                    >
                    <span class="ml-1 font-sans font-normal text-content-3">{{ t('modelPlaza.table.cacheReadShort') }}</span>
              ...
              <div
                ...
                class="space-y-0.5 font-mono text-xs text-content"
              >
                <div>
                  <span class="mr-1 font-sans font-normal text-content-3">{{ t('modelPlaza.table.cacheWrite') }}</span>
                  {{ paidPerMillion(m.pricing?.cache_write_price, period)
                  }}<template v-if="m.pricing?.cache_write_1h_price != null"
                    ><span class="font-sans font-normal text-content-3"> (1h </span>{{ paidPerMillion(m.pricing.cache_write_1h_price, period)
                    }}<span class="font-sans font-normal text-content-3">)</span></template
                  >
                </div>
                <div>
                  <span class="mr-1 font-sans font-normal text-content-3">{{ t('modelPlaza.table.cacheRead') }}</span>
          ...
          <template v-else>
            <td :colspan="showSiteCache ? 3 : 2" class="pz-cell px-3 py-2.5 align-middle">
              <div
              ...
              >
                <span
                  ...
                  :key="idx"
                  class="inline-flex items-center gap-1 rounded-md bg-surface-3 px-2 py-0.5 font-mono text-xs text-content"
                >
                  <span class="font-sans text-content-3">{{ tierLabel(iv) }}</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  }}<span class="font-sans text-content-3">{{ perUnitSuffix(m) }}</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                <span class="font-mono font-semibold text-content dark:text-gray-50">
                  {{ paidRequestPrice(m, m.pricing.per_request_price) }}
                </span>
                <span class="ml-1 text-xs text-content-3">{{ perUnitSuffix(m) }}</span>
              </template>
              <span v-else class="text-content-3">-</span>
            </td>
          </template>
          ...
          <template v-if="showOfficial">
          <td
            class="border-l border-divider px-3 py-2.5 align-middle font-mono text-xs text-content-2"
          >
            <template v-if="officialIntervals(m).length">
              ...
              >
                <span class="mr-1 font-sans text-content-3" :title="t('modelPlaza.table.tierHint')">{{ tierLabel(iv) }}</span>
                {{ official(iv.input_price) }}
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.input_price) }}</template>
          </td>
          <td class="px-3 py-2.5 align-middle font-mono text-xs text-content-2">
            <template v-if="officialIntervals(m).length">
              <div
                v-for="(iv, idx) in officialIntervals(m)"
                :key="idx"
          ...
          <td class="px-3 py-2.5 align-middle">
            <template v-if="hasTierCachePricing(officialIntervals(m))">
              <div
                v-for="(iv, idx) in officialIntervals(m)"
                :key="idx"
                class="whitespace-nowrap font-mono text-xs leading-5 text-content-2"
                :title="t('modelPlaza.table.tierHint')"
              >
                <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                  <span class="font-sans text-content-3">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                  {{ official(iv.cache_write_price) }}
                  <template v-if="iv.cache_write_1h_price != null"
                    ><span class="font-sans text-content-3"> (1h </span>{{ official(iv.cache_write_1h_price)
                    }}<span class="font-sans text-content-3">)</span></template
                  >
                  <span class="ml-1 font-sans text-content-3">{{ t('modelPlaza.table.cacheReadShort') }}</span>
            ...
            <div
              ...
              class="space-y-0.5 font-mono text-xs text-content-2"
            >
              <div>
                <span class="mr-1 font-sans font-normal text-content-3">{{ t('modelPlaza.table.cacheWrite') }}</span>
                {{ official(m.official_pricing.cache_write_price)
                }}<template v-if="m.official_pricing.cache_write_1h_price != null"
                  ><span class="font-sans text-content-3"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
                  }}<span class="font-sans text-content-3">)</span></template
                >
              </div>
              <div>
          ...
          <td
            class="border-l border-divider py-2.5 pl-3 pr-5 text-right align-middle font-mono text-xs"
          >
            <span
              v-if="period"
            ...
            <span
              ...
              class="font-bold text-content-2"
              >{{ requestRate(m) }}x</span
            >
            <template v-else-if="hasCustomRate">
              <span class="mr-1 text-content-3 line-through">{{ rateMultiplier }}x</span>
              <span class="font-bold text-primary-600 dark:text-primary-400">{{ effectiveRate }}x</span>
            </template>
            <span v-else class="font-bold text-content-2">{{ effectiveRate }}x</span>
          </td>
        </tr>
      </tbody>
    </table>
...
/**
 * 列优先级降级（设计稿方案 A）。按表格容器实际宽度判断，不是视口——
 * 控制台侧栏常驻占位，视口宽度会高估可用空间。
 *
 *   ≥1280  全部 8 列
 *   <1280  隐藏「官方价格」整组 —— 它是参考信息不是决策信息，隐藏代价最小
 *   <1024  再隐藏站内「缓存」列
 *
 * colgroup / colspan / 数据行三处必须同步，少改一处表格就会错位。
 */
const tableWrapRef = ref<HTMLElement | null>(null)
const { showWideColumns: showOfficial, showMediumColumns: showSiteCache } =
  useContentBreakpoint(tableWrapRef)
