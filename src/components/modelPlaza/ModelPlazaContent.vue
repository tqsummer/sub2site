<template>
  <div class="space-y-5">
    ...
    <div v-if="!embedded">
      <h1 class="text-2xl font-bold tracking-tight text-content sm:text-3xl">{{ t('modelPlaza.title') }}</h1>
      <p class="mt-1.5 text-sm text-content-2">{{ t('modelPlaza.description') }}</p>
    </div>

    <!-- 全局价格说明(管理员配置,Markdown) -->
    <div
      v-if="descriptionHtml"
      class="plaza-description rounded-2xl border border-divider bg-surface px-5 py-4 text-sm shadow-card"
    ...
    <p
      ...
      class="flex items-center gap-1.5 text-xs text-content-3"
    >
    ...
    <div
      v-else-if="error"
      class="rounded-2xl border border-red-200 bg-danger-soft px-5 py-8 text-center text-sm text-danger dark:border-red-500/30"
    >
    ...
    <template v-else>
      ...
      <div
        v-else
        class="rounded-2xl border border-dashed border-divider-strong px-5 py-12 text-center text-sm text-content-2"
      >
...
.plaza-description :deep(h3) {
  @apply mb-2 mt-3 font-semibold text-content first:mt-0 dark:text-white;
}

.plaza-description :deep(p) {
  @apply mb-2 text-content-2 last:mb-0 dark:text-dark-200;
}
...
.plaza-description :deep(blockquote) {
  @apply my-2 border-l-4 border-gray-300 pl-3 text-content-2 dark:border-dark-600 dark:text-dark-300;
}
</style>
