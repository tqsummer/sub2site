<template>
  <div class="bg-gray-50/50 dark:bg-dark-700/30">
    ...
    <div v-else-if="items.length === 0" class="py-2 text-center text-xs text-content-3">
    ...
    <table v-else class="w-full text-xs">
      <tbody>
        ...
        >
          <td class="max-w-[120px] truncate py-1 pl-6 text-content-2" :title="user.email">
            {{ user.email || `User #${user.user_id}` }}
          </td>
          <td class="py-1 text-right text-content-2">
          ...
          <td class="py-1 pr-1 text-right text-content-3">
