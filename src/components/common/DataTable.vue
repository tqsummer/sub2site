<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-divider bg-surface p-4">
        <div class="space-y-3">
          ...
          <div v-if="hasActionsColumn" class="border-t border-divider pt-3">
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-divider bg-surface p-12 text-center">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-content-3"
            />
    ...
    <template v-else>
      <div v-if="selectable" class="flex items-center justify-end gap-2 px-1">
        <label class="flex items-center gap-2 text-sm font-medium text-content-2">
          <input
            type="checkbox"
            class="h-4 w-4 rounded border-divider-strong text-primary-600 focus:ring-primary-500 dark:bg-dark-800"
      ...
      >
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-content-2">
              {{ column.label }}
            </span>
            <div class="min-w-0 max-w-full text-right text-sm text-content">
  ...
  >
    <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
      <thead class="table-header bg-surface-2">
        <tr>
          ...
          <th
            ...
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-content-2',
      ...
      <tbody class="table-body divide-y divide-gray-200 bg-surface dark:divide-dark-700">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-if="selectable" class="w-11 min-w-11 px-3 py-4">
            <div class="mx-auto h-4 w-4 animate-pulse rounded bg-surface-3"></div>
          </td>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-content-2', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            ...
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-content',
                getAdaptivePaddingClass(),
