<template>
  <div>
    <label class="input-label">
      ...
      <span class="font-normal text-content-3">{{ t('common.selectedCount', { count: modelValue.length }) }}</span>
    </label>
    <div
      v-if="isSearchable"
      class="flex items-center gap-2 rounded-t-lg border border-b-0 border-divider bg-surface-2 px-3 py-2"
    >
      <Icon name="search" size="sm" class="shrink-0 text-content-3" />
      <input
        v-model="searchText"
        type="text"
        :placeholder="t('common.searchPlaceholder')"
        class="flex-1 bg-transparent text-sm text-content placeholder:text-content-3 focus:outline-none dark:text-gray-100 dark:placeholder:text-content-2"
      />
    </div>
    <div
      :class="[
    ...
    >
      ...
      >
        <input
          ...
          class="h-3.5 w-3.5 shrink-0 rounded border-divider-strong text-primary-500 focus:ring-primary-500"
        />
        <GroupBadge
          :name="group.name"
        ...
        />
        <span class="shrink-0 text-xs text-content-3">{{ group.account_count || 0 }}</span>
      </label>
      <div
        v-if="filteredGroups.length === 0"
        class="col-span-2 py-2 text-center text-sm text-content-2"
      >
