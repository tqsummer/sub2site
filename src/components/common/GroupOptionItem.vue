<template>
  <div class="flex min-w-0 flex-1 items-start justify-between gap-3">
    ...
    >
      ...
      <span
        v-if="description"
        class="mt-1.5 w-full whitespace-pre-line [overflow-wrap:anywhere] text-left text-xs leading-relaxed text-content-2 line-clamp-3"
      >
        {{ description }}
      </span>
    </div>
    ...
    <div class="flex shrink-0 items-center gap-2 pt-0.5">
      <div class="flex shrink-0 flex-col items-end gap-1">
        ...
        <span
          v-if="hasPeakRate"
          class="inline-flex items-center whitespace-nowrap rounded-full bg-warning-soft px-3 py-1 text-xs font-semibold text-warning"
          :title="peakRateTitle"
        >
          {{ peakRateText }}
        </span>
