<template>
  ...
  <div
    ...
    class="mt-0.5 flex items-center gap-1 text-xs text-content-3"
  >
  ...
  <div v-else-if="entry.status === 'success'" class="mt-0.5 flex items-center gap-1 text-xs">
    <button
      type="button"
      class="truncate text-content-2 underline decoration-dotted underline-offset-2 hover:text-primary-600 dark:hover:text-primary-400"
      :title="tooltipText"
    ...
    <button
      type="button"
      class="text-content-3 hover:text-primary-600 dark:hover:text-primary-400"
  ...
  <div v-else-if="entry.status === 'error'" class="mt-0.5 text-xs">
    <button
      type="button"
      class="text-danger underline decoration-dashed underline-offset-2 hover:text-danger dark:hover:text-red-300"
      @click="handleFetch"
    >
  ...
  <div v-else class="mt-0.5 text-xs text-content-3">
