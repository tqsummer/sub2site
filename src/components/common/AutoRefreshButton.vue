<template>
  <div class="relative" ref="dropdownRef">
    <button
      ...
      class="inline-flex items-center gap-1.5 rounded-lg border border-divider bg-surface px-2.5 py-1.5 text-xs font-medium text-content-2 shadow-sm transition-colors hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-dark-700"
    ...
    <div
      v-if="showDropdown"
      class="absolute right-0 z-20 mt-1 w-44 rounded-lg border border-divider bg-surface shadow-lg"
    >
      <div class="p-1.5">
        <button
          @click="$emit('update:enabled', !enabled)"
          class="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm text-content-2 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-dark-700"
        >
