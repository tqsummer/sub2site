<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <!-- Logo 插槽：高度锁 28px，宽度自适应但不超过 4:1，
           这样横版长图不会把站名挤出可视区。缺图时降级为首字母方块。 -->
      <router-link
        :to="homePath"
        class="sidebar-logo flex shrink-0 items-center transition-opacity hover:opacity-80"
      ...
      >
        ...
        <span
          v-else
          aria-hidden="true"
          class="grid h-7 w-7 place-items-center rounded-token-sm text-sm font-semibold"
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link
          :to="homePath"
          class="sidebar-brand-title block truncate text-[15px] font-medium transition-colors"
    ...
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide">
      <!-- Regular User View -->
      <template v-if="!appStore.backendModeEnabled">
    ...
    <!-- Bottom Section -->
    <div class="mt-auto border-t border-divider p-3">
      <!-- Theme Toggle -->
      <button
        @click="toggleTheme"
      ...
      >
        <SunIcon v-if="isDark" class="h-5 w-5 flex-shrink-0 text-warning" />
...
/** 无 Logo 时的降级方块取站名首字母；站名也为空时兜底 'A'。 */
