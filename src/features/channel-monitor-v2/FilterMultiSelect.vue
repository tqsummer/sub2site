<template>
  ...
  >
    ...
    >
      ...
      >
        <!-- 内容区窄时省掉「平台：」这类前缀，只留选中值/计数。
             aria-label 上仍有完整 label，读屏不受影响。 -->
        {{ labelHidden ? selectionLabel : t('channelMonitorV2.filters.labelValue', { label, value: selectionLabel }) }}
      </span>
      <span class="select-icon shrink-0 text-content-3 transition-transform" :class="isOpen ? 'rotate-180' : ''">
    ...
    <Teleport to="body">
      <Transition name="select-dropdown">
        ...
        >
          <button
            type="button"
            class="dropdown-item select-option select-option-group flex w-full items-center justify-between border-b border-divider px-4 py-2 text-left text-sm font-semibold text-content-2 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
            @click="clear"
          >
            <span>{{ allLabel }}</span>
          ...
          <button
            ...
            type="button"
            role="option"
            class="dropdown-item select-option flex w-full items-center justify-between gap-3 px-4 py-2 text-left text-sm text-content-2 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
          ...
          >
            <span class="flex min-w-0 flex-1 items-center gap-2">
              <span
                class="checkbox flex h-4 w-4 items-center justify-center rounded border border-divider-strong bg-surface text-primary-500"
            ...
            <small v-if="option.count != null" class="text-xs text-content-3">{{ formatCount(option.count) }}</small>
          </button>
          <p v-if="options.length === 0" class="px-4 py-3 text-center text-xs text-content-3">{{ t('channelMonitorV2.filters.empty') }}</p>
        </div>
      </Transition>
    </Teleport>
  </div>
...
const props = withDefaults(
  defineProps<{
    ...
    compact?: boolean
    /** 收起「标签：」前缀，只显示选中值。窄内容区下用。 */
    labelHidden?: boolean
  }>(),
  { compact: false, labelHidden: false },
)
