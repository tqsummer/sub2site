<template>
  ...
  >
    <div class="card-header mb-4 flex shrink-0 flex-wrap items-start justify-between gap-3 !border-0 !p-0">
      <div class="min-w-0">
        <h2 class="flex items-center gap-2 text-sm font-bold text-content">
      ...
      <div class="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-xs text-content-2 sm:w-auto">
        <span class="badge badge-gray shrink-0">{{ bucketLabel }}</span>
        <span class="hidden text-[11px] text-content-3 sm:inline">{{ t('channelMonitorV2.matrix.wheelZoomX') }}</span>
        <button
          type="button"
          class="inline-flex shrink-0 items-center rounded-lg border border-divider bg-surface px-2 py-1 text-[11px] font-semibold text-content-2 hover:bg-gray-50 disabled:opacity-50 dark:text-gray-300 dark:hover:bg-dark-800"
          :disabled="!zoomed"
    ...
    <div class="card-body min-h-0 flex-1 !p-0">
      <div
        v-if="rows.length"
        ref="scrollRef"
        class="matrix-scroll max-h-[min(42vh,420px)] max-w-full overflow-auto rounded-2xl bg-surface-2 p-2"
        @wheel="onMatrixWheel"
      >
        <div class="matrix-table w-full" :style="tableStyle">
          <div
            class="matrix-header matrix-row sticky top-0 z-[3] bg-surface-2 text-[10px] font-semibold uppercase tracking-wide text-content-2"
          ...
          <div
            ...
            class="matrix-row border-b border-divider"
            :class="showThroughput ? 'matrix-row--with-tps' : ''"
          >
            <div class="dimension-cell flex min-w-0 items-center gap-2 bg-surface" :title="rowLabel(entry.row)">
              <span :class="['status-dot', cellClass(entry.row.health, entry.row.metrics.request_count)]"></span>
              <strong class="truncate text-xs font-semibold text-content">{{ rowLabel(entry.row) }}</strong>
            </div>
            <strong class="summary-value bg-surface text-xs font-medium tabular-nums text-content-2 dark:text-gray-300">
              {{ successRate(entry.row.metrics) }}
            </strong>
            <strong
              class="summary-value bg-surface text-xs font-medium tabular-nums text-content-2 dark:text-gray-300"
      ...
      <div class="mt-4 flex flex-col gap-2" :aria-label="t('channelMonitorV2.matrix.legendAria')">
        <div class="flex items-center gap-2 text-[11px] text-content-2">
        ...
        <div class="flex flex-wrap gap-4 text-[11px] text-content-2">
