<template>
  <section class="mx-auto w-full max-w-6xl space-y-5 px-1 py-2 sm:px-2">
    <header
      class="page-header mb-0 flex flex-wrap items-center justify-between gap-3 rounded-3xl bg-surface p-5 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700 sm:p-6"
    >
      <div class="min-w-0">
        <h2 class="page-title flex items-center gap-2 text-xl font-black text-content">
        ...
        <p class="page-description mt-1.5 text-xs text-content-2">
    ...
    <div
      v-if="!systemModeV2"
      class="rounded-2xl border border-amber-200 bg-warning-soft px-4 py-3 text-sm text-amber-900 dark:border-amber-800/50"
      role="status"
    >
      {{
    ...
    <div
      v-if="loading"
      class="card flex min-h-[200px] items-center justify-center !rounded-3xl !border-0 text-sm text-content-3 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700"
    >
    ...
    <template v-else-if="draft">
      <div class="card divide-y divide-gray-100 !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:divide-dark-700 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.enableTitle') }}</strong>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
          <div>
            <strong class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.refreshTitle') }}</strong>
            <p class="mt-0.5 text-xs text-content-2">{{ t('channelMonitorV2.settings.refreshHint') }}</p>
          </div>
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.platformsTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-2">
        ...
        <div class="divide-y divide-gray-100 dark:divide-dark-700">
          ...
          >
            ...
            <strong class="text-sm font-medium text-content">{{ platformLabel(platform.platform) }}</strong>
            <input
              class="input"
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header flex flex-wrap items-center justify-between gap-2 !py-3">
          <div>
            <h3 class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.groupsTitle') }}</h3>
            <p class="mt-0.5 text-xs text-content-2">
              {{
        ...
        <div class="max-h-[min(40vh,280px)] overflow-y-auto px-3 py-2 sm:px-4">
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            ...
            >
              <input
                type="checkbox"
                class="h-4 w-4 rounded border-divider-strong text-primary-600 focus:ring-primary-500/40"
                :checked="draft.group_ids.includes(group.id)"
                @change="toggleGroup(group.id)"
              />
              <span class="min-w-0 flex-1 truncate font-medium text-content">{{ group.name }}</span>
              <small class="shrink-0 text-xs text-content-3">{{ platformLabel(group.platform) }} · #{{ group.id }}</small>
            </label>
          </div>
          <p v-if="groups.length === 0" class="empty-state py-8 text-sm text-content-3">{{ t('channelMonitorV2.settings.groupsEmpty') }}</p>
        </div>
      </div>

      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.errorsTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-2">
        ...
        <div class="max-h-[min(40vh,320px)] overflow-y-auto px-3 py-2 sm:px-4">
          <div class="grid grid-cols-1 gap-1 sm:grid-cols-2">
            ...
            >
              ...
              />
              <span class="min-w-0 flex-1 truncate font-medium text-content">
                {{ categoryLabel(category) }}
              </span>
              <small class="shrink-0 font-mono text-[10px] text-content-3">{{ category }}</small>
            </label>
          </div>
        </div>
        <div class="border-t border-divider px-5 py-3 text-xs text-content-2">
          {{
      ...
      <div class="card overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="card-header !py-3">
          <h3 class="text-sm font-semibold text-content">{{ t('channelMonitorV2.settings.healthTitle') }}</h3>
          <p class="mt-0.5 text-xs text-content-2">
      ...
      <div class="space-y-2">
        ...
        <div class="rounded-2xl border border-divider bg-surface-2 px-4 py-3 text-xs text-content-2 dark:text-gray-300">
          <p class="font-medium text-content">{{ t('channelMonitorV2.settings.userContractTitle') }}</p>
