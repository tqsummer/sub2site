<template>
  <AppLayout>
    <div class="mx-auto max-w-[1600px]" :class="activeTab === 'config' && draft ? 'pb-28' : 'pb-8'">
      <header class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 class="mt-1 text-2xl font-semibold tracking-tight text-content">{{ t('admin.promptAudit.title') }}</h1>
          <p class="mt-2 max-w-3xl text-sm text-content-2 dark:text-dark-300">{{ t('admin.promptAudit.description') }}</p>
        </div>
        <div v-if="draft" class="text-right text-xs text-content-2">
      ...
      <div v-if="loadErrors.config && !draft" role="alert" class="rounded-xl border border-red-200 bg-danger-soft p-5 dark:border-red-900">
        <p class="text-sm text-danger">{{ loadErrors.config }}</p>
        ...
        <main class="card px-4 sm:px-6 lg:px-8">
          <div v-show="activeTab === 'config'" data-test="tab-panel-config">
            ...
            <template v-if="draft">
              <EndpointPool
                ...
                @probe="runProbe"
              />
              <div v-if="loadErrors.groups" role="alert" class="mt-5 rounded-lg bg-warning-soft px-4 py-3 text-sm text-amber-800">{{ loadErrors.groups }}</div>
          ...
          <div v-show="activeTab === 'events'" data-test="tab-panel-events">
            <div
              ...
              role="status"
              class="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-warning-soft px-4 py-3 text-sm text-amber-900 dark:border-amber-900/70"
            >
    ...
    <div v-if="draft && activeTab === 'config'" class="fixed inset-x-0 bottom-0 z-30 border-t border-divider bg-white/95 px-4 py-3 shadow-[0_-12px_35px_rgba(15,23,42,0.08)] backdrop-blur dark:bg-dark-900/95 dark:shadow-[0_-12px_35px_rgba(0,0,0,0.35)] lg:left-64">
      <div class="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3">
          <span class="text-sm" :class="dirty ? 'text-warning' : 'text-content-2'">
...
const SaveToggle = defineComponent({
  ...
  setup(props, { emit, attrs }) {
    return () => h('label', { class: ['flex items-center gap-2.5 text-sm', props.disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'] }, [
      ...
      h('span', { class: 'select-none text-content-2' }, props.label),
    ])
  },
})
