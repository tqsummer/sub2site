<template>
  <section aria-labelledby="prompt-events-title" class="py-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 id="prompt-events-title" class="text-base font-semibold text-content">{{ t('admin.promptAudit.events.title') }}</h2>
        <p class="mt-1 text-sm text-content-2 dark:text-dark-300">{{ t('admin.promptAudit.events.description') }}</p>
      </div>
    ...
    <div v-if="error" role="alert" class="mt-4 rounded-lg bg-danger-soft px-4 py-3 text-sm text-danger">{{ error }}</div>
    <div class="mt-5 overflow-x-auto rounded-xl border border-divider">
      <table class="min-w-[1120px] w-full text-left text-sm">
        <thead class="bg-surface-2 text-xs uppercase tracking-wide text-content-2">
          <tr>
        ...
        <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-700 dark:bg-transparent">
          <tr v-if="loading"><td colspan="8" class="px-4 py-12 text-center text-content-2" aria-busy="true">{{ t('common.loading') }}</td></tr>
          <tr v-else-if="events.length === 0"><td colspan="8" class="px-4 py-12 text-center text-content-2">{{ t('admin.promptAudit.events.empty') }}</td></tr>
          <tr v-for="event in events" v-else :key="event.id" :data-test="`event-${event.id}`" class="align-top hover:bg-gray-50/70 dark:hover:bg-dark-800/70">
            ...
            <td class="whitespace-nowrap px-3 py-3 text-xs text-content-2">{{ formatDate(event.created_at) }}</td>
            <td class="px-3 py-3">
            ...
            <td class="px-3 py-3 text-content-2">{{ event.snapshot.group_name || '—' }}</td>
            <td class="px-3 py-3">
              <p class="font-medium text-content">{{ event.snapshot.endpoint }}</p>
              <p class="mt-1 text-xs text-content-2">{{ event.snapshot.model }} · {{ event.snapshot.protocol }} · {{ event.snapshot.stage || 'http' }}</p>
            </td>
            <td class="px-3 py-3">
              ...
              <p class="mt-2 max-w-48 truncate text-xs text-content-2" :title="formatCategories(event.categories)">{{ formatCategories(event.categories) }}</p>
            </td>
            <td class="max-w-xs px-3 py-3"><p class="line-clamp-2 break-words text-content-2">{{ event.snapshot.redacted_preview || '—' }}</p></td>
...
const FilterInput = defineComponent({
  ...
  setup(componentProps, { emit: componentEmit }) {
    return () => h('label', { class: 'text-xs text-content-2 dark:text-dark-200' }, [
...
const CopyLine = defineComponent({
  ...
  setup(componentProps) {
    return () => h('div', { class: 'flex max-w-56 items-center gap-1 text-xs' }, [
      h('span', { class: 'w-16 flex-none text-content-2' }, componentProps.label),
      h('span', { class: 'min-w-0 flex-1 truncate text-content' }, componentProps.value || '—'),
...
function decisionClass(decision: string): string {
  if (decision === 'critical') return 'bg-red-100 text-danger'
  if (decision === 'flag') return 'bg-amber-100 text-warning'
