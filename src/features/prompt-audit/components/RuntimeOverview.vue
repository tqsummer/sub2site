<template>
  <section aria-labelledby="prompt-runtime-title" class="border-b border-divider py-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 id="prompt-runtime-title" class="text-base font-semibold text-content">
          {{ t('admin.promptAudit.runtime.title') }}
        </h2>
        <p class="mt-1 text-sm text-content-2 dark:text-dark-300">
    ...
    <div v-if="error" role="alert" class="mt-5 rounded-lg bg-danger-soft px-4 py-3 text-sm text-danger">
      {{ error }}
    </div>
    <div v-else-if="loading && !runtime" class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" aria-busy="true">
      <div v-for="index in 6" :key="index" class="h-16 animate-pulse rounded-xl bg-surface-3" />
    </div>
    <template v-else-if="runtime">
      <dl class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <div v-for="item in statusItems" :key="item.label" class="rounded-xl border border-divider bg-surface-2 px-3 py-3">
          <dt class="text-xs text-content-2">{{ item.label }}</dt>
          <dd class="mt-1.5 flex items-center gap-2 text-sm font-semibold text-content">
      ...
      <div class="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.6fr)]">
        <div class="rounded-xl border border-divider px-4 py-3 dark:bg-dark-900/20">
          <h3 class="text-sm font-medium text-content">{{ t('admin.promptAudit.runtime.guardMetrics') }}</h3>
          <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div v-for="metric in guardMetricItems" :key="metric.label" class="rounded-lg bg-surface-2 px-2.5 py-2">
              <p class="text-[11px] text-content-2">{{ metric.label }}</p>
              <p class="mt-0.5 text-sm font-semibold tabular-nums text-content">{{ metric.value }}</p>
            </div>
          </div>
          <p class="mt-3 text-xs leading-5 text-content-2">
        ...
        <div class="rounded-xl border border-divider px-4 py-3 dark:bg-dark-900/20">
          <h3 class="text-sm font-medium text-content">{{ t('admin.promptAudit.runtime.latest') }}</h3>
          ...
          <p v-if="runtime.last_error_code" class="mt-1 break-words text-sm text-danger">
