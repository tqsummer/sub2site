<template>
  <BaseDialog :show="show" :title="t('admin.promptAudit.events.filterDeleteDialogTitle')" width="wide" @close="$emit('close')">
    <div class="space-y-5 text-sm">
      <p class="text-content-2 dark:text-dark-300">{{ t('admin.promptAudit.events.filterDeleteDialogDesc') }}</p>

      <fieldset>
        <legend class="text-xs font-medium text-content-2 dark:text-dark-200">{{ t('admin.promptAudit.events.filterTimeRange') }}</legend>
        <div class="mt-2 flex flex-wrap gap-2" role="radiogroup" :aria-label="t('admin.promptAudit.events.filterTimeRange')">
          ...
          >
            ...
            <span class="inline-flex items-center rounded-full border border-divider px-3 py-1.5 text-xs font-medium text-content-2 transition-colors peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700 peer-focus-visible:ring-2 peer-focus-visible:ring-red-500/30 dark:text-dark-300 dark:peer-checked:border-red-500 dark:peer-checked:bg-red-950/40 dark:peer-checked:text-red-300">
              {{ t(`admin.promptAudit.events.timePresets.${option.id}`) }}
            </span>
          </label>
        </div>
        <p class="mt-2 text-xs text-content-2">{{ t('admin.promptAudit.events.filterTimeRangeHint') }}</p>
        <div v-if="preset === 'custom'" class="mt-3 grid gap-3 sm:grid-cols-2" data-test="custom-range">
          <label class="text-xs text-content-2 dark:text-dark-200">
          ...
          <p v-if="!canPreview" class="text-xs text-danger sm:col-span-2">{{ t('admin.promptAudit.events.customRangeInvalid') }}</p>
        </div>
      </fieldset>
      ...
      <details class="rounded-xl border border-divider px-4 py-3" data-test="more-conditions">
        <summary class="cursor-pointer select-none text-xs font-medium text-content-2 dark:text-dark-200">{{ t('admin.promptAudit.events.moreConditions') }}</summary>
      ...
      <div v-if="preview" class="rounded-xl border border-red-200 bg-danger-soft px-4 py-3 dark:border-red-900/60" data-test="delete-preview-result">
        <p class="text-sm font-semibold text-danger">{{ t('admin.promptAudit.events.filterDeleteCount', { count: preview.matched_count }) }}</p>
        <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs text-content-2">
        ...
        <p class="mt-2 rounded-lg bg-warning-soft px-3 py-2 text-xs text-amber-800">{{ t('admin.promptAudit.events.filterDeleteWarning') }}</p>
      </div>
      <p v-else class="rounded-xl border border-dashed border-divider-strong px-4 py-3 text-xs text-content-2" data-test="delete-preview-empty">
    ...
    <template #footer>
      <div class="flex flex-wrap items-center justify-end gap-3">
        <p v-if="confirmDisabledReason" class="mr-auto text-xs text-content-2" data-test="confirm-disabled-reason">
