<template>
  <section aria-labelledby="prompt-pool-title" class="border-b border-divider py-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 id="prompt-pool-title" class="text-base font-semibold text-content">{{ t('admin.promptAudit.pool.title') }}</h2>
        <p class="mt-1 text-sm text-content-2 dark:text-dark-300">{{ t('admin.promptAudit.pool.description') }}</p>
      </div>
    ...
    <div v-if="endpoints.length === 0" class="mt-5 rounded-xl border border-dashed border-divider-strong px-5 py-10 text-center text-sm text-content-2 dark:bg-dark-900/20 dark:text-dark-300">
      {{ t('admin.promptAudit.pool.empty') }}
    </div>
    <div v-else class="mt-5 overflow-hidden rounded-xl border border-divider bg-surface">
      <div class="hidden grid-cols-[minmax(260px,1.45fr)_minmax(210px,1fr)_minmax(190px,.8fr)_minmax(230px,1.15fr)_auto] gap-5 border-b border-l-[3px] border-b-gray-200 border-l-transparent bg-surface-2 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-content-2 dark:border-b-dark-700/60 xl:grid">
        ...
        >
          <div class="flex min-w-0 items-center gap-3">
            <button
              ...
              :class="endpoint.enabled ? 'bg-primary-600' : 'bg-surface-3'"
            ...
            <div class="min-w-0">
              <div class="flex min-w-0 items-center gap-2">
                <p class="truncate font-semibold text-content">{{ endpoint.name }}</p>
              ...
              <p class="mt-0.5 truncate font-mono text-[11px] text-content-2" :title="endpoint.base_url">{{ endpoint.base_url }}</p>
            </div>
          </div>

          <div class="min-w-0 xl:block">
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-content-3 xl:hidden">{{ t('admin.promptAudit.pool.model') }}</p>
            <p class="truncate text-sm font-medium text-content-2" :title="endpoint.model">{{ endpoint.model }}</p>
          </div>

          <div>
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-content-3 xl:hidden">{{ t('admin.promptAudit.pool.limits') }}</p>
            <div class="flex flex-wrap gap-1.5 text-xs text-content-2">
              <span class="rounded-md bg-surface-3 px-2 py-1 tabular-nums">{{ endpoint.timeout_ms }} ms</span>
              <span class="rounded-md bg-surface-3 px-2 py-1 tabular-nums">{{ endpoint.input_limit }} chars</span>
            </div>
          </div>

          <div class="min-w-0">
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-content-3 xl:hidden">{{ t('admin.promptAudit.pool.credential') }}</p>
            <div class="flex items-center gap-1.5 text-xs font-medium" :class="credentialInvalid(endpoint) ? 'text-danger' : hasCredential(endpoint) ? 'text-success' : 'text-content-2'">
            ...
            <p v-if="probeResults[endpoint.id]" class="mt-1.5 line-clamp-2 text-xs leading-5" :class="probeResults[endpoint.id].ok ? 'text-success' : 'text-danger'">
          ...
          <div class="flex flex-wrap items-center justify-end gap-1 border-t border-divider pt-3 xl:flex-nowrap xl:border-0 xl:pt-0">
            ...
            <button type="button" class="btn btn-ghost btn-sm text-danger hover:bg-red-50 dark:hover:bg-red-950/30" @click="removeEndpoint(endpoint)">{{ t('common.delete') }}</button>
          </div>
        </article>
      </div>
    </div>
    ...
    <BaseDialog :show="Boolean(editing)" :title="editingIndex < 0 ? t('admin.promptAudit.pool.add') : t('admin.promptAudit.pool.edit')" width="wide" @close="closeEditor">
      <form v-if="editing" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveEditor">
        <label class="space-y-1 text-sm text-content-2">
        ...
        <label class="space-y-1 text-sm text-content-2 sm:col-span-2">
        ...
        <label class="space-y-1 text-sm text-content-2 sm:col-span-2">
          ...
          <span class="block text-xs text-content-2">{{ t('admin.promptAudit.pool.secretHint') }}</span>
        </label>
