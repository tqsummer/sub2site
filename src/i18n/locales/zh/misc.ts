export default {
  ...
  payment: {
    ...
    // 空态：设计稿要求给出「为什么不可用 + 怎么办」，不能只留一句「暂未开放」
    notAvailableDesc: '当前没有可用的支付方式。',
    notAvailableContact: '如需充值，请联系客服：{contact}',
    // 余额与支付币种分属两层：余额恒为 USD，支付币种由所选渠道决定。
    // 界面上必须分别标出，否则用户无从判断扣的是哪种钱。
    balanceCurrencyNote: '当前余额 · USD',
    payCurrencyNote: '支付币种 {currency}',
