...
// LandingView 直接从 @/stores/auth 取，单独 mock 一份
vi.mock('@/stores/auth', () => ({
  useAuthStore: () => authStore,
}))

// 落地页挂载时会拉模型广场；这里挡掉，厂商带与价格表在测试中不渲染
...
describe('HomeView compact mode', () => {
  ...
  // sub2site 剥离了管理端，/admin/* 路由不存在，管理员同样落到用户控制台
  it('links administrators to the user dashboard', () => {
    authStore.isAuthenticated = true
