<template>
  <AppLayout>
      ...
      <OrderTable :orders="orders" :loading="loading">
        <template #actions="{ row }">
          <div class="flex items-center gap-2">
            <button v-if="row.status === 'PENDING'" @click="handleCancel(row.id)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-warning hover:bg-yellow-50 dark:hover:bg-yellow-900/20">
              <Icon name="x" size="sm" />
    ...
    <BaseDialog :show="!!cancelTargetId" :title="t('payment.orders.cancel')" width="narrow" @close="cancelTargetId = null">
      <p class="text-sm text-content-2">{{ t('payment.confirmCancel') }}</p>
      <template #footer>
      ...
      <div v-if="refundTarget" class="space-y-4">
        <div class="rounded-xl bg-surface-2 p-4">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-mono text-content">#{{ refundTarget.id }}</span>
          </div>
          <div class="mt-2 flex justify-between text-sm">
            <span class="text-content-2">{{ t('payment.orders.amount') }}</span>
            <span class="text-content">${{ refundTarget.amount.toFixed(2) }}</span>
          </div>
        </div>
        <div>
