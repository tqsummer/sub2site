<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <div v-if="qrUrl" class="rounded-2xl bg-surface p-6 shadow-lg">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-content-2">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-content-2">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-content">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-content-3">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
