  ...
  <div class="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-slate-950">
    ...
    >
      ...
      <div v-if="amount" class="text-center">
        ...
        <p v-if="orderId" class="mt-1 text-sm text-content-2 dark:text-slate-400">
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-lg border border-red-200 bg-danger-soft p-3 text-sm text-danger dark:border-red-700"
        >
          {{ error }}
        </div>
        <button
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        ...
        <p class="text-sm text-content-2 dark:text-slate-400">{{ t('payment.result.success') }}</p>
        <button
      ...
      <div v-else class="flex items-center justify-center py-8">
        ...
        />
        <span class="ml-3 text-sm text-content-2 dark:text-slate-400">{{ hint }}</span>
      </div>
    </div>
  </div>
</template>
