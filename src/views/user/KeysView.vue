  ...
  <AppLayout>
        ...
        <div class="flex justify-end gap-3">
          ...
          <div class="relative" ref="columnDropdownRef">
            ...
            <div
              ...
              class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-divider bg-surface py-1 shadow-lg"
            >
              <button
                v-for="col in toggleableColumns"
                :key="col.key"
                @click="toggleColumn(col.key)"
                class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-content-2 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <span>{{ col.label }}</span>
                <Icon
      ...
      <template #table>
        ...
        >
          <template #cell-id="{ value }">
            <span class="font-mono text-xs text-content-2">#{{ value }}</span>
          </template>
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                :class="
                  copiedKeyId === row.id
                    ? 'text-success'
                    : 'text-content-3 hover:text-content-2 dark:hover:text-gray-300'
                "
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              ...
              >
                ...
                />
                <span v-else class="text-sm text-content-3">{{
                  t('keys.noGroup')
                }}</span>
                <span class="text-xs text-content-2">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-content-3 opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
            ...
            <span
              :class="[
                ...
                (value ?? 0) > 0
                  ...
                  : 'bg-gray-100 text-content-2 dark:bg-dark-700'
              ]"
            >
              {{ value ?? 0 }}
            </span>
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-content-2">{{ t('keys.today') }}:</span>
                <span class="font-medium text-content">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-content-2">{{ t('keys.total') }}:</span>
                <span class="font-medium text-content">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-content-2">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
                    row.quota_used >= row.quota ? 'text-danger' :
                    row.quota_used >= row.quota * 0.8 ? 'text-warning' :
                    'text-content'
                  ]">
          ...
          <template #cell-rate_limit="{ row }">
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-content-2">5h</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_5h >= row.rate_limit_5h ? 'text-danger' :
                    row.usage_5h >= row.rate_limit_5h * 0.8 ? 'text-warning' :
                    'text-content-2'
                  ]">
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-content-3 tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-content-2">1d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_1d >= row.rate_limit_1d ? 'text-danger' :
                    row.usage_1d >= row.rate_limit_1d * 0.8 ? 'text-warning' :
                    'text-content-2'
                  ]">
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-content-3 tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-content-2">7d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_7d >= row.rate_limit_7d ? 'text-danger' :
                    row.usage_7d >= row.rate_limit_7d * 0.8 ? 'text-warning' :
                    'text-content-2'
                  ]">
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-content-3 tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-content-2 transition-colors hover:bg-gray-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
            ...
            <span v-else class="text-sm text-content-3">-</span>
          </template>

          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-danger' : 'text-content-2'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-content-3">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-content-2">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-content-3">-</span>
          </template>
          ...
          <template #cell-created_at="{ value }">
            <span class="text-sm text-content-2">{{ formatDateTime(value) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-success"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'text-content-2 hover:bg-yellow-50 hover:text-yellow-600 dark:hover:bg-yellow-900/20 dark:hover:text-warning'
                    : 'text-content-2 hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-success'
                ]"
              >
              ...
              <button
                @click="editKey(row)"
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-gray-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-danger"
              >
      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-content-3">{{ t('keys.selectGroup') }}</span>
            </template>
        ...
        <div v-if="!showEditModal" class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-primary-600' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.use_custom_key">
            ...
            />
            <p v-if="customKeyError" class="mt-1 text-sm text-danger">{{ customKeyError }}</p>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_ip_restriction ? 'bg-primary-600' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_quota ? 'bg-primary-600' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-content-2">$</span>
                <input
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-surface-3 px-3 py-2">
                  <span class="font-medium text-content">
                    ${{ selectedKey.quota_used?.toFixed(4) || '0.0000' }}
                  </span>
                  <span class="mx-2 text-content-3">/</span>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-primary-600' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-surface-3 px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h ? 'text-danger' :
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h * 0.8 ? 'text-warning' :
                      'text-content'
                    ]">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-surface-3 px-3 py-2 text-sm">
                    <span :class="[
                      ...
                      selectedKey.usage_1d >= selectedKey.rate_limit_1d * 0.8 ? 'text-warning' :
                      'text-content'
                    ]">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-surface-3 px-3 py-2 text-sm">
                    <span :class="[
                      ...
                      selectedKey.usage_7d >= selectedKey.rate_limit_7d * 0.8 ? 'text-warning' :
                      'text-content'
                    ]">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_expiration ? 'bg-primary-600' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
              ...
              <button
                ...
                :class="[
                  ...
                  formData.expiration_preset === days
                    ...
                    : 'bg-gray-100 text-content-2 hover:bg-surface-3 dark:hover:bg-dark-600'
                ]"
              >
            ...
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-content-2">{{ t('keys.currentExpiration') }}: </span>
              <span class="font-medium text-content">
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-divider hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-content-2" />
	            <span class="font-medium text-content">{{
	              t('keys.ccsClientSelect.claudeCode')
	            }}</span>
	            <span class="text-xs text-content-2">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-divider hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-content-2" />
	            <span class="font-medium text-content">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max max-w-[calc(100vw-16px)] overflow-hidden rounded-xl bg-surface shadow-lg ring-1 ring-black/5 duration-200 sm:min-w-[380px] dark:ring-white/10"
      ...
      >
        ...
        <div class="border-b border-divider p-2">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-content-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-lg border border-divider bg-surface-2 py-1.5 pl-8 pr-3 text-sm text-content placeholder-gray-400 outline-none focus:border-primary-300 focus:ring-1 focus:ring-primary-300 dark:text-white dark:placeholder-gray-500 dark:focus:border-primary-600 dark:focus:ring-primary-600"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-content-3">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
