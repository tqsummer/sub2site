<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="flex flex-col gap-3">
          ...
          <div
            ...
            class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-divider bg-surface px-3 py-2 shadow-sm"
          >
            <i18n-t
              ...
              class="text-sm text-content-2"
            >
              <template #count>
                <span class="font-medium text-content">{{ selectedJobIds.size }}</span>
              </template>
            </i18n-t>
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 rounded border-divider-strong text-primary-600 focus:ring-primary-500"
          ...
          <template #cell-id="{ row }">
	            <div class="flex w-[220px] items-start gap-1" :class="row.is_child ? 'pl-6' : ''">
	              <button
	                ...
	                type="button"
	                class="mt-1 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md text-content-2 transition-colors hover:bg-gray-100 hover:text-content focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:hover:bg-dark-700 dark:hover:text-white"
	              ...
	              <button type="button" class="min-w-0 flex-1 rounded-lg py-1 text-left transition-colors hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:hover:bg-dark-700" @click="selectJob(row.id)">
	                <span
	                  ...
	                  :class="row.task_name ? 'text-content' : 'text-content-2'"
                >
                  <span class="min-w-0 truncate">{{ row.task_name || defaultTaskName(row.created_at) }}</span>
                  <span v-if="row.child_count > 0 && !row.is_child" class="flex-shrink-0 rounded-full bg-surface-3 px-2 py-0.5 text-xs font-normal text-content-2 dark:text-gray-300">
                    {{ t('batchImage.list.childCount', { n: row.child_count }, row.child_count) }}
                  </span>
                  <span v-if="row.is_child" class="flex-shrink-0 rounded-full bg-warning-soft px-2 py-0.5 text-xs font-normal text-warning">
                    {{ t('batchImage.list.childBadge') }}
                  </span>
	                </span>
	                <span class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-content-2">
          ...
          <template #cell-model="{ row }">
	            <div class="mx-auto max-w-[180px] text-center">
	              <p class="truncate text-sm text-content-2" :title="row.model">{{ row.model }}</p>
	            </div>
	          </template>

          <template #cell-api_key_name="{ value }">
            <span class="block truncate text-center text-sm text-content-2">
          ...
          <template #cell-counts="{ row }">
            <div class="flex items-center justify-center gap-2 text-sm tabular-nums">
              <span class="text-success">{{ displayJob(row).success_count }}</span>
              <span class="text-gray-300">/</span>
              <span :class="displayJob(row).fail_count > 0 ? 'text-danger' : 'text-content-3'">{{ displayJob(row).fail_count }}</span>
              <span class="text-xs text-content-3">{{ t('batchImage.list.totalCount', { n: displayJob(row).item_count }) }}</span>
            </div>
          </template>

          <template #cell-cost="{ row }">
            <span class="block text-center text-sm text-content-2">
              {{ costLabel(displayJob(row)) }}
            </span>
          </template>

          <template #cell-downloaded="{ row }">
            <span class="block text-center text-sm" :class="row.downloaded_at ? 'text-success' : 'text-content-2'">
              ...
              <button
                type="button"
                class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-gray-100 hover:text-primary-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:hover:bg-dark-700 dark:hover:text-primary-400"
              ...
              <button
                ...
                :class="canDownload(row) ? 'text-content-2 hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-green-400' : 'text-gray-300'"
              ...
              <div v-if="canRetry(row) || canDeleteRecord(row)">
                <button
                  type="button"
                  class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-content-2 transition-colors hover:bg-gray-100 hover:text-content focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:hover:bg-dark-700 dark:hover:text-white"
                  :class="{ 'bg-gray-100 text-content dark:bg-dark-700 dark:text-white': openMoreJobId === row.id }"
          ...
          <template #empty>
            <div class="flex min-h-[260px] flex-col items-center justify-center py-6 md:min-h-[300px]">
              <Icon name="sparkles" size="xl" class="mb-4 h-12 w-12 text-content-3" />
              <p class="text-lg font-medium text-content">{{ t('batchImage.list.empty') }}</p>
      ...
      <template #pagination>
        <div
          ...
          class="flex flex-col gap-3 border-t border-divider bg-surface px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6"
        >
          <div class="flex flex-wrap items-center gap-3 text-sm text-content-2">
    ...
    <Teleport to="body">
      <div
        v-if="openMoreJobId"
        class="fixed z-[9999] w-44 overflow-hidden rounded-xl bg-surface py-1 text-sm shadow-lg ring-1 ring-black/5 dark:ring-white/10"
        :style="moreMenuStyle"
        @click.stop
      >
        <template v-for="job in batchJobs" :key="job.id">
          <template v-if="job.id === openMoreJobId">
            <button
              v-if="canRetry(job)"
              type="button"
              class="flex w-full items-center gap-2 px-3 py-2 text-left text-content-2 transition-colors hover:bg-amber-50 hover:text-amber-700 disabled:opacity-60 dark:text-gray-200 dark:hover:bg-amber-900/20 dark:hover:text-amber-300"
            ...
            <button
              ...
              type="button"
              class="flex w-full items-center gap-2 px-3 py-2 text-left text-danger transition-colors hover:bg-red-50 disabled:opacity-60 dark:hover:bg-red-900/20"
    ...
    <Teleport to="body">
      <div
        ...
        class="batch-prompt-popover fixed z-[9999] rounded-lg border border-divider bg-surface p-3 text-sm text-content shadow-xl ring-1 ring-black/5 dark:ring-white/10"
        :style="promptPopover.style"
        @mouseenter="cancelPromptPopoverClose"
        @mouseleave="schedulePromptPopoverClose"
      >
        <div class="mb-2 flex items-center justify-between gap-3">
          <span class="text-xs font-medium text-content-2">{{ t('batchImage.promptPopover.title') }}</span>
          <button
            type="button"
    ...
    <BaseDialog :show="!!currentJob" :title="t('batchImage.detail.title')" width="extra-wide" @close="closeDetail">
      <div v-if="currentJob" class="space-y-4">
        <div class="rounded-lg border border-divider bg-surface-2 px-4 py-3">
          <div class="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-2">{{ t('common.status') }}</p>
            ...
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-2">{{ hasChildJobs(currentJob.id) ? t('batchImage.detail.aggregatedResult') : t('batchImage.detail.result') }}</p>
              <p class="mt-1 flex items-center justify-center gap-2 font-medium tabular-nums">
              <span class="text-success">{{ (currentDisplayJob || currentJob).success_count }}</span>
              <span class="text-gray-300">/</span>
              <span :class="(currentDisplayJob || currentJob).fail_count > 0 ? 'text-danger' : 'text-content-3'">{{ (currentDisplayJob || currentJob).fail_count }}</span>
            </p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-2">{{ t('batchImage.detail.cost') }}</p>
              <p class="mt-1 truncate font-medium text-content">{{ costLabel(currentDisplayJob || currentJob) }}</p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-content-2">{{ t('batchImage.detail.downloadStatus') }}</p>
              <p class="mt-1 truncate font-medium text-content">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h3 class="text-sm font-semibold text-content">{{ t('batchImage.detail.items') }}</h3>
        ...
        <div v-if="items.length" class="overflow-x-auto rounded-lg border border-divider bg-surface">
          <table class="w-full min-w-[860px] table-fixed divide-y divide-gray-200 text-sm dark:divide-dark-700">
            ...
            <thead class="bg-surface-2">
              <tr>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-2">Custom ID</th>
                <th class="px-3 py-3 text-left text-sm font-medium text-content-2">Prompt</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-2">{{ t('common.status') }}</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-2">{{ t('batchImage.detail.preview') }}</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-content-2">{{ t('batchImage.detail.result') }}</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-3 py-2.5 text-center">
                  <span
                    ...
                    :class="isRecoveredOriginalFailure(item) ? 'text-content-3' : 'text-content'"
                    :title="item.custom_id"
                  >
                    {{ item.custom_id }}
                  </span>
                </td>
                <td class="px-3 py-2.5 text-left" :class="isRecoveredOriginalFailure(item) ? 'text-content-3' : 'text-content-2'">
                  <div
                ...
                <td class="px-3 py-2.5 text-center">
                  <div class="mx-auto h-12 w-12 overflow-hidden rounded-md border border-divider bg-surface-2">
                    <button
                    ...
                    <button
                      ...
                      type="button"
                      class="flex h-full w-full items-center justify-center text-content-2 transition-colors hover:bg-gray-100 hover:text-primary-600 disabled:cursor-wait disabled:opacity-70 dark:hover:bg-dark-700"
                    ...
                    <div v-else class="flex h-full w-full items-center justify-center text-content-3" :title="item.image_count > 0 ? t('batchImage.detail.previewUnavailable') : t('batchImage.detail.noImage')">
        ...
        <div v-else class="rounded-lg border border-dashed border-divider py-10 text-center">
          <Icon name="refresh" size="lg" class="mx-auto mb-3 text-content-3" :class="loadingItems ? 'animate-spin' : ''" />
          <p class="text-sm font-medium text-content-2">
            {{ loadingItems ? t('batchImage.detail.loadingItems') : t('batchImage.detail.noItems') }}
          </p>
          <p v-if="!loadingItems" class="mt-1 text-sm text-content-2">
    ...
    <BaseDialog :show="!!previewImageItem" :title="previewImageItem?.custom_id || t('batchImage.imagePreview.title')" width="extra-wide" :z-index="60" @close="closeImagePreview">
      <div class="space-y-3">
        <div class="rounded-lg border border-amber-200 bg-warning-soft px-3 py-2 text-sm text-amber-900 dark:border-amber-800">
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <p v-if="!loadingKeys && geminiApiKeys.length === 0" class="input-hint text-warning">
          ...
          <div>
            ...
            <p v-if="modelLoadError" class="input-hint text-warning">
              {{ modelLoadError }}
            </p>
            <p v-else-if="selectedApiKey && !loadingModels && availableBatchImageModels.length === 0" class="input-hint text-warning">
          ...
          <div>
            ...
            <div class="input flex items-center bg-surface-2 text-content-2 dark:text-gray-300">
              1K
            </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-content-2">{{ t('batchImage.create.promptAdded', { count: promptRows.length }) }}</span>
          </div>
          <div class="rounded-lg border border-divider p-3">
            <textarea
              v-model="promptDraft"
              rows="3"
              class="h-[76px] w-full resize-y rounded-md border border-divider-strong px-3 py-2 text-sm leading-5 outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-100 dark:bg-dark-900 dark:text-gray-100 dark:focus:border-primary-500 dark:focus:ring-primary-900/40"
            ...
            <div v-if="referenceImageDrafts.length" class="mt-3 flex flex-wrap gap-2">
              <span
                ...
                class="inline-flex max-w-full items-center gap-1 rounded-md border border-divider bg-surface-2 px-2 py-1 text-xs text-content-2 dark:text-gray-200"
              >
                <span class="max-w-[180px] truncate">{{ ref.name }}</span>
                <button type="button" class="text-content-3 hover:text-red-600" :title="t('batchImage.create.removeReferenceImage')" @click="removeReferenceImageDraft(refIndex)">
                  <Icon name="x" size="xs" />
                </button>
              </span>
            </div>
            <p class="mt-2 text-xs text-content-2">
          ...
          <div v-if="promptRows.length" class="overflow-hidden rounded-lg border border-divider">
            <div
              v-for="(row, index) in promptRows"
              :key="row.localId"
              class="flex items-center gap-3 border-b border-divider px-3 py-2 last:border-b-0"
            >
              <span class="w-20 flex-shrink-0 font-mono text-xs text-content-2">{{ row.custom_id }}</span>
              <p class="min-w-0 flex-1 truncate text-sm text-content">{{ row.prompt }}</p>
              <span v-if="row.output_count > 1" class="flex-shrink-0 text-xs text-content-2">
                x{{ row.output_count }}
              </span>
              <span v-if="row.reference_images.length" class="flex-shrink-0 text-xs text-content-2">
                {{ t('batchImage.create.referenceCount', { n: row.reference_images.length }, row.reference_images.length) }}
              </span>
              <button type="button" class="btn-ghost btn-icon flex-shrink-0 text-danger hover:bg-red-50 dark:hover:bg-red-900/20" :title="t('common.delete')" @click="removePromptRow(index)">
                <Icon name="trash" size="sm" />
              </button>
            </div>
          </div>
          <div v-else class="rounded-lg border border-dashed border-divider px-3 py-6 text-center text-sm text-content-2">
            {{ t('batchImage.create.noPrompts') }}
          </div>
        </div>

	        <div class="rounded-lg border border-amber-200 bg-warning-soft p-3 text-sm leading-6 text-amber-900 dark:border-amber-800">
    ...
    <BaseDialog :show="showGuideModal" :title="t('batchImage.guide.title')" width="wide" @close="showGuideModal = false">
	      <div class="space-y-5">
	        <section class="space-y-3">
	          <h3 class="text-sm font-semibold text-content">{{ t('batchImage.guide.uiTitle') }}</h3>
	          <div class="rounded-lg border border-divider bg-surface-2 p-3 text-sm leading-6 text-content-2 dark:text-gray-200">
	        ...
	        <section class="space-y-3">
	          <div class="flex flex-wrap items-center justify-between gap-3">
	            <h3 class="text-sm font-semibold text-content">{{ t('batchImage.guide.skillTitle') }}</h3>
	            <p class="text-xs text-content-2">{{ t('batchImage.guide.skillDesc') }}</p>
	          </div>
	        <textarea
	          :value="agentInstruction"
	          readonly
	          class="min-h-[420px] w-full resize-y rounded-md border border-divider bg-surface-2 p-4 font-mono text-sm leading-6 text-content outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 dark:focus:border-primary-500 dark:focus:ring-primary-900/40"
	        />
	        </section>
	      </div>
      <template #footer>
...
function detailItemRowClass(item: BatchImageDetailItem) {
  if (isRecoveredOriginalFailure(item)) {
    return 'bg-gray-50/80 text-content-3 hover:bg-gray-100/80 dark:bg-dark-900/60 dark:hover:bg-dark-800/70'
  }
...
function itemResultClass(item: BatchImageDetailItem) {
  if (isRecoveredOriginalFailure(item)) return 'bg-gray-100 text-content-2 ring-gray-200 dark:bg-dark-800 dark:ring-dark-700'
  ...
  return 'bg-gray-50 text-content-2 ring-gray-200 dark:bg-dark-800 dark:ring-dark-700'
}
