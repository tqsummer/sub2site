<template>
  <AppLayout>
    <!-- ref 供 useContentBreakpoint 观测：按内容区实际宽度分档，
         不能用 window.innerWidth —— 左侧侧栏常驻占 260/72px，视口宽度会高估可用空间。 -->
    <div ref="pageRef" class="space-y-6 pb-12">
      <!-- 卡片外观交给 .card（扁平 + 发丝描边），这里不再用毛玻璃与 3xl 圆角覆盖 -->
      <section class="card sticky top-0 z-20 p-0">
        <header class="page-header mb-0 flex flex-wrap items-start justify-between gap-4 border-b border-divider px-5 py-4 sm:px-6">
          <div class="min-w-0">
            <h1 class="page-title flex items-center gap-2">
              <span class="inline-flex h-8 w-8 items-center justify-center rounded-token bg-brand-soft text-brand-text">
                <Icon name="chart" size="sm" />
              </span>
              {{ t('channelMonitorV2.title') }}
            </h1>
            <div class="page-description mt-1.5 flex flex-wrap items-center gap-2 text-xs text-content-2">
              <span class="relative flex h-2 w-2 shrink-0">
                <span
                  class="relative inline-flex h-2 w-2 rounded-full"
                  :class="loading || refreshing ? 'bg-surface-3' : 'bg-success'"
                ></span>
              </span>
              <span v-if="refreshing" class="inline-flex items-center gap-1 text-brand-text">
              ...
              <span v-else class="text-content-3">{{ t('common.loading') }}</span>
              <span
        ...
        <div
          v-if="bootstrapActive"
          class="border-b border-divider bg-brand-soft px-5 py-3 sm:px-6"
          role="status"
          aria-live="polite"
        >
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-0.5 text-xs text-content-2">
                {{ t('channelMonitorV2.bootstrap.description') }}
              </p>
            </div>
            <span class="shrink-0 text-xs font-medium tabular-nums text-brand-text">
              {{ t('channelMonitorV2.bootstrap.progress', { percent: bootstrapPercent }) }}
            </span>
          </div>
          <div
        ...
        <!-- 工具栏控件多，窄内容区必然横向滚动。
             两端加渐变遮罩，否则用户不知道右边还有内容（原来只有 overflow-x-auto，无任何提示）。 -->
        <div class="monitor-toolbar-wrap relative">
        <div class="monitor-toolbar flex flex-nowrap items-center gap-1.5 overflow-x-auto px-4 py-3 sm:gap-2 sm:px-5">
          ...
          <span class="mx-0.5 hidden h-5 w-px shrink-0 bg-surface-3 sm:block" aria-hidden="true"></span>

          <FilterMultiSelect
            v-model="filter.platforms"
            compact
            :label-hidden="!showFilterLabels"
          ...
          <span class="mx-0.5 hidden h-5 w-px shrink-0 bg-surface-3 md:block" aria-hidden="true"></span>
      ...
      <div class="relative min-h-[320px]">
        ...
        <div
          v-else-if="loading"
          class="card flex min-h-[320px] items-center justify-center !rounded-3xl !border-0 text-sm text-content-3 shadow-sm ring-1 ring-gray-900/5 dark:ring-dark-700"
        >
      ...
      <section class="card flex min-h-0 flex-col overflow-hidden !rounded-3xl !border-0 shadow-sm ring-1 ring-gray-900/5 dark:!bg-dark-800 dark:ring-dark-700">
        <div class="border-b border-divider px-5 pt-4 sm:px-6">
        ...
        <div class="min-h-0 max-h-[min(52vh,520px)] overflow-auto p-4 sm:p-5">
                ...
                >
                  <td>
                    <div class="flex items-center gap-2">
                      ...
                      <div>
                        <span class="block text-xs text-content-2">{{ row.platform }}</span>
                  ...
                  <td>
                    ...
                    <small class="text-xs text-content-3">{{ t('channelMonitorV2.metrics.errorRateValue', { value: formatPercent(row.metrics.error_rate) }) }}</small>
                  </td>
                  <td>
                    <span class="block">{{ formatMs(row.metrics.ttft.p50_ms) }}</span>
                    <small class="text-xs text-content-3">{{ latencyDetail(row.metrics.ttft) }}</small>
                  </td>
          ...
          <div v-else-if="activeTab === 'errors'" class="space-y-3">
            ...
            >
              ...
              >
                <span class="flex min-w-0 items-center gap-1.5 truncate text-content-2">
                ...
                <small
                  ...
                  :class="row.ignored ? 'text-content-3' : 'text-content-2'"
                >{{ formatPercent(row.rate) }}</small>
                <Icon name="chevronDown" size="sm" :class="['text-content-3 transition-transform', expandedErrors.has(row.category) ? 'rotate-180' : '']" />
              </button>
              <div v-if="expandedErrors.has(row.category)" class="mt-3 space-y-2 border-t border-divider pt-3">
                <template v-if="isAdmin && (row.details || []).length">
                  <div
                    v-for="(detail, index) in row.details || []"
                    :key="`${row.category}:${index}:${detail.message}`"
                    class="rounded-lg bg-surface-2 px-3 py-2 text-xs text-content-2 dark:text-dark-300"
                  >
                    <div class="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span v-if="detail.status_code" class="text-content-3">{{ t('channelMonitorV2.errorDetail.http', { code: detail.status_code }) }}</span>
                      <span v-if="detail.upstream_status_code" class="text-content-3">{{ t('channelMonitorV2.errorDetail.upstream', { code: detail.upstream_status_code }) }}</span>
                      <span class="ml-auto text-content-3">×{{ detail.count }}</span>
                    </div>
                    <p class="break-words leading-relaxed">{{ detail.message || detail.error_type || t('channelMonitorV2.errorDetail.noMessage') }}</p>
                  </div>
                </template>
                <p v-else class="text-xs text-content-3">{{ t('channelMonitorV2.errorDetail.empty') }}</p>
              </div>
            </div>
          </div>
              ...
              <tbody>
                ...
                >
                  ...
                  <td>
                    <strong
                      class="font-semibold"
                      :class="row.is_self ? 'text-primary-700 dark:text-primary-300' : 'text-content'"
                    >
                      {{ row.display_label }}
                      <span
                        v-if="row.is_self"
          ...
          <div v-if="tabLoading" class="empty-state py-10 text-sm text-content-3">{{ t('common.loading') }}</div>
...
// 观测内容区实际宽度用于分档（侧栏常驻，视口宽度会高估可用空间）
const pageRef = ref<HTMLElement | null>(null)
const { showFilterLabels } = useContentBreakpoint(pageRef)
...
<style scoped>
/* 工具栏横向滚动的两端渐变遮罩。
   没有它的话，窄内容区下右侧被截断的控件完全没有视觉提示。
   用 ::before/::after 而不是额外 DOM，避免影响 flex 布局。 */
.monitor-toolbar-wrap::before,
.monitor-toolbar-wrap::after {
  content: '';
  position: absolute;
  top: 0;
  bottom: 0;
