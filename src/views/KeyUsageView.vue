<template>
  <div class="relative flex min-h-screen flex-col bg-surface-2">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-content">{{ siteName }}</span>
        </router-link>
        <div class="flex items-center gap-3">
          ...
          <a
            ...
            class="rounded-lg p-2 text-content-2 transition-colors hover:bg-gray-100 hover:text-content-2 dark:hover:bg-dark-800 dark:hover:text-white"
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-content">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-content-2 text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-content-3">
            ...
            <input
              ...
              class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-divider bg-surface text-sm text-content placeholder:text-content-3 transition-all dark:text-white dark:placeholder:text-content-3"
              @keydown.enter="queryKey"
            />
            <button
              @click="keyVisible = !keyVisible"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-content-3 hover:text-content-2 dark:hover:text-white transition-colors"
            >
        ...
        <p class="text-xs text-content-3 mt-3 text-center">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-content-2">{{ t('keyUsage.dateRange') }}</span>
            <button
              ...
              :class="currentRange === range.key
                ...
                : 'border-gray-200 bg-white text-content-2 dark:border-dark-700 dark:bg-dark-900 dark:text-dark-200 hover:border-primary-300 dark:hover:border-dark-600'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                v-model="customStartDate"
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-divider bg-surface text-content dark:text-white"
              />
              <span class="text-xs text-content-3">-</span>
              <input
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-2xl border border-divider bg-surface p-8">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-divider bg-white/90 shadow-sm backdrop-blur-sm dark:bg-dark-900/90">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-content">{{ statusInfo.label }}</span>
              <span class="text-xs text-content-3">|</span>
              <span class="text-xs text-content-2">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-2xl border border-divider bg-white/90 p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-lg dark:bg-dark-900/90"
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-content-2">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-content-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-content-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-content-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-content">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-content-2 mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-content-3 mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 rounded-2xl border border-divider bg-white/90 backdrop-blur-sm overflow-hidden dark:bg-dark-900/90"
          >
            <div class="px-8 py-5 border-b border-divider">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-dark-800">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-content-2">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-content'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-divider">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-surface-3">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-surface px-6 py-4"
              >
                <div class="text-xs text-content-2 mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-content">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 rounded-2xl border border-divider bg-white/90 backdrop-blur-sm overflow-hidden dark:bg-dark-900/90"
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-divider sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-divider bg-surface p-0.5">
                <button
                  ...
                  :class="dailyUsageDays === option.value
                    ...
                    : 'text-content-2 hover:bg-gray-100 dark:text-dark-300 dark:hover:bg-dark-800'"
                >
                  {{ option.label }}
                </button>
              </div>
              ...
              <table class="w-full">
                <thead>
                  <tr class="border-b border-divider bg-surface-2">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                  ...
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-content">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-content">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-content-2">
          ...
          >
            <div class="px-8 py-5 border-b border-divider">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-divider bg-surface-2">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-2">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-divider last:border-b-0"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-content">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-content-2">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-content">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-divider px-6 py-8">
