<template>
  <div class="min-h-screen bg-surface-2 text-content dark:text-white">
    <header class="border-b border-divider bg-white/95 dark:bg-dark-900/95">
      <div class="mx-auto flex max-w-5xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <RouterLink to="/home" class="flex min-w-0 items-center gap-3">
          <template v-if="settings">
            <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl bg-surface shadow-sm ring-1 ring-gray-200 dark:ring-dark-700">
          ...
          <template v-else>
            <span class="h-10 w-10 flex-shrink-0 animate-pulse rounded-xl bg-surface-3" aria-hidden="true"></span>
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:py-10">
      ...
      <section
        v-else-if="loadError"
        class="rounded-lg border border-red-200 bg-danger-soft p-6 text-danger dark:border-red-500/30"
      >
      ...
      <section
        ...
        class="rounded-lg border border-divider bg-surface p-6"
      >
        <div class="flex items-start gap-3">
          <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md bg-surface-3 text-content-2 dark:text-dark-300">
            <Icon name="document" size="sm" />
          </span>
          <div>
            <h1 class="text-lg font-semibold text-content">{{ t('legal.notFound') }}</h1>
      ...
      <article v-else>
        <div class="mb-8 border-b border-divider pb-6">
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0">
              ...
              <h1 class="mt-2 break-words text-2xl font-bold tracking-normal text-content sm:text-3xl">
                {{ currentDocument.title }}
              </h1>
              <p v-if="updatedAt" class="mt-3 text-sm text-content-2">
        ...
        <div
          v-else
          class="rounded-lg border border-dashed border-divider-strong bg-surface px-6 py-14 text-center text-sm text-content-2"
        >
          {{ t('legal.empty') }}
        </div>
      </article>
...
.legal-document-content :deep(li) {
  @apply mb-1 text-content-2;
}

.legal-document-content :deep(blockquote) {
  @apply my-5 border-l-4 border-gray-300 pl-4 text-content-2 dark:border-dark-600 dark:text-dark-300;
}
